/**
 * Friends: decline a pending friend request.
 *
 * POST { requesterSteamId } -> proxied to the bridge Worker's
 * /friend-decline route. steamId comes from the verified session, not a
...
export async function onRequestPost(context) {
  ...
  try {
    const response = await fetch(`${origin}/friend-decline`, {
      method: 'POST',
      headers: {
